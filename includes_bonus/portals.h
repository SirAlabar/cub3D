...
/*   Updated: 2025/03/24 19:46:38 by hluiz-ma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool						is_valid_portal_surface(t_game *game,
								t_portal_hit hit);

/* portal_creation.c */
void						create_portal(t_game *game, t_portal_hit hit,
								t_portal_type type);
...
void						translate_portal_ray(t_ray *ray, t_game *game,
								t_portal_wall *entry, t_portal_wall *exit);

/* portal_integration.c */
void						check_ray_portal_collision(t_ray *ray,
								t_game *game);
void						integrate_portals_with_raycasting(t_game *game,
								t_ray *rays);
void						update_ray_check_collisions(t_ray *ray,
								t_game *game);
void						integrate_portal_system(t_game *game);

bool						check_portal_hit(t_ray *ray, t_game *game,
								t_portal_wall *portal);
unsigned int				get_portal_color(t_portal_wall *portal, double x,
								double y, t_game *game);
