...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/22 21:01:12 by hluiz-ma          #+#    #+#             */
/*   Updated: 2025/01/22 21:01:12 by hluiz-ma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <cub3d.h>

static double	get_wall_distance(int check_pos, double new_pos,
		double grid_pos)
{
...
static bool	check_wall_collision(t_game *g, int x, int y)
{
	...
	if (tile == 'D')
	{
		...
		if (door)
		{
			player_x = (int)g->p1.pos.x;
			player_y = (int)g->p1.pos.y;
			if (player_x == x && player_y == y)
				return (false);
			if (door->state == DOOR_OPEN || (door->state == DOOR_OPENING
					&& door->animation >= 0.8) || (door->state == DOOR_CLOSING
					&& door->animation >= 0.2))
				return (false);
		}
		return (true);
...
bool	can_move_x(t_game *g, t_vector new, t_vector dir, double buff)
{
	...
	{
		dist = get_wall_distance(check_x, new.x, new_x_pos);
		if (dist < buff)
		{
			return (false);
		}
...
bool	can_move_y(t_game *g, t_vector new, t_vector dir, double buff)
{
	...
	{
		dist = get_wall_distance(check_y, new.y, new_y_pos);
		if (dist < buff)
		{
			return (false);
		}
